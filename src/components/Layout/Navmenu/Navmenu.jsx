import React from 'react';
import s from './Navmenu.module.css';
import {NavLink} from "react-router-dom";
console.log(s);

const  Navmenu = (props) => {
    return (
        <nav className={s.nav}>
            <div className={s.nav_user}>
                <img className={s.nav_user_img} src="data:image/png;base64,"/>
                <h3>User Name:</h3>
                <p>@username</p>
            </div>
            <ul>
                <li><NavLink to="/home" activeClassName={s.active}> Home</NavLink></li>
                <li><NavLink to="/dialogs" activeClassName={s.active}>Message</NavLink></li>
                <li><NavLink to="/posts" activeClassName={s.active}>Posts</NavLink></li>
                <li><NavLink to="/about" activeClassName={s.active}>About</NavLink></li>
                <li><NavLink to="/photos" activeClassName={s.active}>Photos</NavLink></li>
                <li><NavLink to="/reviews" activeClassName={s.active}>Reviews</NavLink></li>
                <li><NavLink to="/events" activeClassName={s.active}>Events</NavLink></li>
                <li><NavLink to="/community" activeClassName={s.active}>Community</NavLink></li>
            </ul>
        </nav>

    );
}

export default Navmenu;